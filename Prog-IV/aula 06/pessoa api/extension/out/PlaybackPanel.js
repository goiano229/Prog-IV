...
(function () {
  ...
  // ── event application ─────────────────────────────────────────────────────────
  /**
   * VS Code stores ranges as 0-based lines/chars; Monaco uses 1-based
   * lineNumber/column. We add 1 when building Monaco Range objects.
   */
  function toMonacoRange(evt) {
    const sl = (evt.rangeStartLine ?? 0) + 1;
    const sc = (evt.rangeStartChar ?? 0) + 1;
    const el = (evt.rangeEndLine  ?? evt.rangeStartLine ?? 0) + 1;
    const ec = (evt.rangeEndChar  ?? evt.rangeStartChar ?? 0) + 1;
  ...
  function applyEvent(evt) {
    ...
    switch (evt.type) {
      ...
      case 'PASTE': {
        ...
        const range = evt.rangeStartLine != null
          ? toMonacoRange(evt)
      ...
      case 'DELETE': {
        if (evt.rangeStartLine == null) { break; }
      ...
      case 'CURSOR_MOVE': {
        if (evt.rangeStartLine == null) { break; }
        const pos = {
          lineNumber: evt.rangeStartLine + 1,
          column: (evt.rangeStartChar ?? 0) + 1,
        };
      ...
      case 'FILE_OPEN': {
        // Reset editor content when the student opens a new file.
        editor.setValue('');
        if (evt.filePath) {
      ...
      // FILE_SAVE has no visual effect during playback.
    }
  }
